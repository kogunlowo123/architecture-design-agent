...
def configure_logging(level: str = "INFO", *, json_output: bool = True) -> None:
    """Configure the ``archagent`` logger tree.
    ...
    """
    logger = logging.getLogger("archagent")
...
def get_logger(name: str) -> logging.Logger:
    """Return a logger namespaced under ``archagent``."""
    return logging.getLogger(name if name.startswith("archagent") else f"archagent.{name}")
